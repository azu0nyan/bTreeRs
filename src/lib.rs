...
//! assert_eq!(tree.tick(&mut bot), Status::Success); // works, energy -> 0
//! assert_eq!(tree.tick(&mut bot), Status::Success); // rests, energy -> 1
...
///     AlwaysSuccess,
/// ]);
/// assert_eq!(seq.tick(&mut ()), Status::Success);
/// ```
#[macro_export]
macro_rules! nodes {
    ($($node:expr),* $(,)?) => {
